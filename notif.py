...
def check_farm_summary():
    ...
    text_farming_status = req[req.find(": ")+2:req.find("\n")]
    return text, text_farming_status, chia
...
if __name__ == '__main__':
    df = read_data()
    text, text_farming_status, chia = check_farm_summary()
    ...
    if text_farming_status != df.loc[0, "last_farming_status"]:
        SendMarkdown(chat_id="-590191599", text="Farming Status: "+text_farming_status, token=os.environ.get("TELEGRAM_API_TOKEN"))
        df.loc[0, 'last_farming_status'] = text_farming_status
        df.to_csv("data/chia.csv", index=False)